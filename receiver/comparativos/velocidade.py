...
pilot_dois = session.laps.pick_driver('VER').pick_fastest()
...
fig, ax = plt.subplots()
ax.plot(piloto_um_tel['Distance'], piloto_um_tel['Speed'], color=equipe_um_color, label='PILOTO UM')
ax.plot(piloto_dois_tel['Distance'], piloto_dois_tel['Speed'], color=equipe_dois_color, label='PILOTO DOIS')

ax.set_xlabel('Distancia em metros')
ax.set_ylabel('Velocidade em km/h')
